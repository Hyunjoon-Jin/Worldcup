...
interface TrendPoint { stage: number; byTeam: Record<string, number> }

/**
 * 대륙컵 우승 확률 추이 라인 차트 — 월드컵 ProbabilityTrendChart와 동형(인라인 SVG, 외부 라이브러리 없음).
 * 단계(조추첨→조별→녹아웃)가 진행될 때마다 쌓인 스냅샷으로 상위 5팀의 우승 확률 변화를 그린다.
 */
export function CupProbabilityTrendChart({ trend }: { trend: TrendPoint[] }) {
  const { lines, maxPct, xLabels } = useMemo(() => {
    if (trend.length < 2) return { lines: [] as { teamId: string; points: { x: number; y: number; pct: number }[] }[], maxPct: 1, xLabels: [] as number[] }
    const latest = trend[trend.length - 1].byTeam
    const topTeams = Object.entries(latest).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([id]) => id)
    const xs = trend.map((h) => h.stage)
    const minX = Math.min(...xs)
    const maxX = Math.max(...xs)
    const spanX = maxX - minX || 1
    let maxPct = 1
    for (const h of trend) for (const t of topTeams) maxPct = Math.max(maxPct, h.byTeam[t] ?? 0)
    const lines = topTeams.map((teamId) => ({
      teamId,
      points: trend.map((h) => ({ x: ((h.stage - minX) / spanX) * 100, y: 100 - ((h.byTeam[teamId] ?? 0) / maxPct) * 100, pct: h.byTeam[teamId] ?? 0 })),
    }))
  ...
  return (
    <GlassCard className="p-4">
      ...
      <div className="mt-1 flex justify-between text-[10px] text-gray-500">
        <span>{xLabels[0]}단계</span>
        <span>최대 {maxPct.toFixed(0)}% · 우승</span>
        <span>{xLabels[1]}단계</span>
      </div>
    </GlassCard>
  )
}
