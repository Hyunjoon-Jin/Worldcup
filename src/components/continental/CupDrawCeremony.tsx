...
export function CupDrawCeremony({
  ...
  const revealCount = useContinentalStore((s) => s.drawRevealCount)
    ...
    <div className="flex flex-col gap-5">
      <GlassCard strong className="p-5 text-center">
        <p className="mb-3 text-sm text-gray-300">
          {done
            ? '조추첨이 완료되었습니다! 각 포트에서 조마다 한 팀씩 배정된 본선 조편성이 확정되었습니다.'
            : next
              ? `포트 ${next.pot + 1}에서 ${groupLetter(next.group)}조에 배정될 국가를 뽑습니다.`
              : ''}
        </p>
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="text-xs text-gray-400">
                    {CONFEDERATION_LABEL_KO[lastTeam.confederation]} · {groupLetter(last.group)}조 · 포트 {last.pot + 1}
                  </div>
                </div>
              </motion.div>
            )}
        ...
        <div className="mb-1 text-[11px] text-gray-500">{revealCount} / {total} 팀 공개</div>
        <div className="mt-2 flex flex-wrap items-center justify-center gap-3">
          {!done ? (
            <GlassButton onClick={revealDrawTeam}>🎱 다음 국가 뽑기</GlassButton>
          ) : (
            <span className="text-xs font-semibold text-emerald-300">✅ 조편성 확정</span>
          )}
      ...
      <GlassCard className="p-4">
        <h3 className="mb-3 text-sm font-bold text-gray-200">🎡 시드 포트 <span className="text-[11px] font-normal text-gray-500">(포트에서 조마다 1팀씩 · 개최국은 포트1 A조 시드)</span></h3>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {drawInfo.pots.map((pot, pi) => (
            <GlassCard key={pi} className={`p-3 transition-all ${!done && next?.pot === pi ? 'ring-2 ring-sky-300/60' : ''}`}>
              ...
              <div className="space-y-1">
                {pot.map((tid) => {
                  ...
                  return (
                    <div key={tid} className={`flex items-center gap-1.5 text-[11px] ${picked ? 'opacity-30' : ''}`}>
                      {team && <FlagIcon iso2={team.iso2} className="h-3 w-4 shrink-0" />}
                      <span className="min-w-0 truncate text-gray-200">{team?.nameKo ?? tid}</span>
                      {host && <span className="shrink-0 text-[8px] text-sky-300">개최</span>}
                      {picked && <span className="ml-auto shrink-0 text-[9px] text-emerald-300/80">{groupLetter(drawInfo.groupOf.get(tid) ?? 0)}조</span>}
                    </div>
                  )
                })}
              </div>
