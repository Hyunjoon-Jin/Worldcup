...
/** 대회 저장 슬롯 UI (D3). 현재 대회를 저장하고 슬롯을 불러오거나 삭제한다. */
export function SaveSlotsPanel() {
  const slots = useSaveSlotsStore((s) => s.slots)
  const saveCurrent = useSaveSlotsStore((s) => s.saveCurrent)
  const load = useSaveSlotsStore((s) => s.load)
  const remove = useSaveSlotsStore((s) => s.remove)
  const champion = useProgressStore((s) => s.champion)
  const groupCount = useProgressStore((s) => s.groupMatches.length)
  const [label, setLabel] = useState('')

  const defaultLabel = champion ? `${TEAMS_BY_ID[champion]?.nameKo ?? champion} 우승` : `진행 중 (${groupCount}경기)`
